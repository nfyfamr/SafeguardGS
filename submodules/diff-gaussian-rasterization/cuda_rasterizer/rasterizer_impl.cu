...
// Overload forward to calculate bleding weight score
int CudaRasterizer::Rasterizer::forwardBWScore(
	std::function<char* (size_t)> geometryBuffer,
	std::function<char* (size_t)> binningBuffer,
	std::function<char* (size_t)> imageBuffer,
	const int P, int D, int M,
	...
	bool debug)
{
	...
	// Let each tile blend its range of Gaussians independently in parallel
	const float* feature_ptr = colors_precomp != nullptr ? colors_precomp : geomState.rgb;
	CHECK_CUDA(FORWARD::bw_score_gaussian(
		tile_grid, block,
		imgState.ranges,
		binningState.point_list,
		width, height,
...
// Overload forward to calculate max weight score
int CudaRasterizer::Rasterizer::forwardMWScore(
	std::function<char* (size_t)> geometryBuffer,
	std::function<char* (size_t)> binningBuffer,
	std::function<char* (size_t)> imageBuffer,
	const int P, int D, int M,
	...
	bool debug)
{
	...
	// Let each tile blend its range of Gaussians independently in parallel
	const float* feature_ptr = colors_precomp != nullptr ? colors_precomp : geomState.rgb;
	CHECK_CUDA(FORWARD::mw_score_gaussian(
		tile_grid, block,
		imgState.ranges,
		binningState.point_list,
		width, height,
...
// Overload forward to calculate top k delta T
int CudaRasterizer::Rasterizer::forwardTopK(
	std::function<char* (size_t)> geometryBuffer,
	std::function<char* (size_t)> binningBuffer,
	std::function<char* (size_t)> imageBuffer,
	const int P, int D, int M,
	...
	bool debug)
{
	...
	// Let each tile blend its range of Gaussians independently in parallel
	const float* feature_ptr = colors_precomp != nullptr ? colors_precomp : geomState.rgb;
	CHECK_CUDA(FORWARD::topk_gaussian(
		tile_grid, block,
		imgState.ranges,
		binningState.point_list,
		width, height,
		...
		binningState_for_opacity.point_list_unsorted,
		out_color), debug)

	if (num_rendered > 0)
	{
		// Sort by keys (descending order of opacity per-tile)
		CHECK_CUDA(cub::DeviceRadixSort::SortPairs(
...
// Overload forward to calculate top k weighted-colors
int CudaRasterizer::Rasterizer::forwardTopKColor(
	std::function<char* (size_t)> geometryBuffer,
	std::function<char* (size_t)> binningBuffer,
	std::function<char* (size_t)> imageBuffer,
	const int P, int D, int M,
...
// Overload forward to calculate top k weights
int CudaRasterizer::Rasterizer::forwardTopKWeight(
	std::function<char* (size_t)> geometryBuffer,
	std::function<char* (size_t)> binningBuffer,
	std::function<char* (size_t)> imageBuffer,
	const int P, int D, int M,
	...
	bool debug)
{
	...
	// Let each tile blend its range of Gaussians independently in parallel
	const float* feature_ptr = colors_precomp != nullptr ? colors_precomp : geomState.rgb;
	CHECK_CUDA(FORWARD::topk_weight_gaussian(
		tile_grid, block,
		imgState.ranges,
		binningState.point_list,
		width, height,
